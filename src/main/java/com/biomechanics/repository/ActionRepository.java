...
public interface ActionRepository extends CrudRepository<Action, Integer> {
    ...
    @Query("FROM Action a INNER JOIN ActionTree at2 ON a.id = at2.actionIdPrev WHERE at2.actionIdNext.id = :actionIdNext")
    List<Action> findByActionIdNext(Integer actionIdNext);

    @Query("FROM Action a INNER JOIN ActionTree at2 ON a.id = at2.actionIdNext WHERE at2.actionIdPrev.id = :actionIdPrev")
